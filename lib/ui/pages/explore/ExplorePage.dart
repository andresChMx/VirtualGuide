...
class _ExplorePageState extends State<ExplorePage>
  ...
  StreamSubscription subscriptionProjects;
  @override
  void initState() {
    super.initState();
    ...
    Future.delayed(Duration.zero, () {
      ...
      //   }
      subscriptionProjects = placesBloc.projectsStream.listen((event) {
        if (event != null) {
          if (event.length > 0) {
            newCameraPosition['lat'] = event[0].lat;
            newCameraPosition['lng'] = event[0].lng;
          }

          mapMarkersList = _createMapMarkersFromProjects(event);
        }
        setState(() {});
      });
    });
  ...
  Widget _buildCarousel(PlacesBloc placesBloc) {
    return StreamBuilder(
      stream: placesBloc.projectsStream,
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.hasData) {
          return Positioned(
              ...
              left: 0,
              right: 0,
              child: ExperiencesCarousel(projects: snapshot.data));
        } else {
          return Container();
        }
      },
                  ...
                  children: places
                      .map((result) => ListTile(
                            ...
                            onTap: () async {
                              ...
                              placesBloc.getNearbyProjects(lat, long);
  ...
  void dispose() {
    ...
    subscriptionProjects.cancel();
  }

  List<Marker> _createMapMarkersFromProjects(List<Project> projects) {
    return projects.map((project) {
      return Marker(
          ...
          visible: true,
          infoWindow:
              InfoWindow(title: project.name, snippet: project.description),
